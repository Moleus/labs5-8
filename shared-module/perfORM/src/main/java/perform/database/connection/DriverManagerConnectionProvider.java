...
public class DriverManagerConnectionProvider implements ConnectionProvider, Configurable {
  ...
  private static Properties getConnectionProperties(Properties properties) {
    ...
    for (Map.Entry<Object, Object> entry : properties.entrySet()) {
      ...
      if (!((String) entry.getKey()).startsWith(AvailableSettings.CONNECTION_PREFIX)) {
        continue;
      }
      if (entry.getKey().equals(AvailableSettings.CONNECTION_PREFIX)) {
        continue;
      }
      connectionProps.setProperty(
          ((String) entry.getKey()).substring(AvailableSettings.CONNECTION_PREFIX.length() + 1),
