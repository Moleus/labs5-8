...
import ru.moleus.kollector.domain.client.CommandExecutor;
import ru.moleus.kollector.domain.collection.CollectionFilter;
import ru.moleus.kollector.domain.commands.readers.DtoReader;
...
public class UserConsole implements Console, IOSource {
  ...
  public UserConsole(PrintStream out, CommandManagerImpl commandManager, Exchanger exchanger, CollectionFilter collectionFilter) {
    this.out = out;
  ...
  private void commandLoop() throws IOException {
    ...
    while (true) {
      ...
      ExecutionPayload payload = ExecutionPayload.of(commandName, inlineArg);
      payload.setData(data);
      payload.setUser(user);
      CommandExecutor commandExecutor = new RemoteCommandExecutor(exchanger);
      commandExecutor.execute(payload, null);
    }
  }
